...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run hwcli",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found hwcli session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["hwcli", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting hwcli...")
        UI.println()

        const opencodeArgs = sessionId ? ["-s", sessionId] : []
        const opencodeProcess = Process.spawn(["hwcli", ...opencodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        ...
        if (code !== 0) throw new Error(`hwcli exited with code ${code}`)
      },
    })
  },
})
